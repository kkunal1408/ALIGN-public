...
void Placer::PlacementRegularAspectRatio_ILP(std::vector<PnRDB::hierNode>& nodeVec, string opath, int effort, PnRDB::Drc_info& drcInfo, bool select_in_ILP = false){
  ...
  if (!designData.isTop && designData.RemoveTaps()) {
    nodeSize *= 2;
  }
  ...
  for (bool removeTaps : {false, true}) {
    ...
    for(std::map<double, std::pair<SeqPair, ILP_solver>>::iterator it=spVec.begin(); it!=spVec.end() && idx<nodeSize; ++it, ++idx) {
      ...
      auto fnstr(opath + nodeVec.back().name + "_" + std::to_string(idx) + (removeTaps ? "_wotap" : ""));
      it->second.second.WritePlacement(designData, it->second.first, fnstr + ".pl");
      it->second.second.PlotPlacement(designData, it->second.first, fnstr + ".plt");

  ...
  // set_add_rowmode(lp, FALSE);
  {
    ...
    for (unsigned int i = 0; i < mydesign.Nets.size(); i++) {
      ...
      if (Lblock_id != Rblock_id) {
        ...
        row.at(mydesign.Blocks.size() * 6 + i * 2 + 1) = 1;
      }
      if (Dblock_id != Ublock_id) {
        {
          double sparserow[7] = {1, 1, 1, -1, -1, -1, -1};
        ...
        row.at(mydesign.Blocks.size() * 6 + i * 2 + 2) = const_graph.LAMBDA;
      }
    }
    ...
    // add estimated area
    for (unsigned int i = 0; i < mydesign.Blocks.size(); i++) row.at(curr_sp.negPair[i] * 6 + 2) += estimated_width / 2;
    // estimate height
    for (unsigned int i = URblock_pos_id; i < curr_sp.posPair.size(); i++) {
      if (curr_sp.posPair[i] < int(mydesign.Blocks.size())) {
    ...
    // add estimated area
    for (unsigned int i = 0; i < mydesign.Blocks.size(); i++) row.at(curr_sp.negPair[i] * 6 + 1) += estimated_height / 2;
